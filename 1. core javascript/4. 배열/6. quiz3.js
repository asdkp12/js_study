...
while (true) {
  deleteMember = prompt (`${tvxq} 삭제할 멤버를 입력하세요`);
 
  check = tvxq.includes(deleteMember);
  
  target = tvxq.indexOf(deleteMember);
  
  if (check) {
    tvxq.splice(target,1);
    alert (`${deleteMember}가 삭제됨`);
  } else {alert (`${deleteMember}은 잘못된 이름입니다. 다시 입력하세요`)}

  if (tvxq.length === 0) break;
}
alert ('모든 멤버가 삭제 됐습니다')
